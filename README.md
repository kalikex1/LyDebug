...
LyDebug 是一款使用C/C++语言开发实现的命令行版R3应用层动态反汇编调试器，运用Windows系统下的调试API函数并配合Capstone反汇编框架完美实现了x86与x64程序的反汇编调试功能，该调试器目前功能包括寄存器查看修改，软硬件断点的设置删除以及遍历，单步步进步过操作，堆栈检查，内存检查等功能。
